...
connection_status = tk.Label(connection_frame, text="🔴 Disconnected", 
...
switch_status = tk.Label(status_grid, text="✅ Normal", font=("Arial", 10), bg='white', fg='green')
...
signal_status = tk.Label(status_grid, text="✅ Normal", font=("Arial", 10), bg='white', fg='green')
...
track_status = tk.Label(status_grid, text="✅ Normal", font=("Arial", 10), bg='white', fg='green')
...
crossing_status = tk.Label(status_grid, text="✅ Normal", font=("Arial", 10), bg='white', fg='green')
...
def initialize_socket_server():
    try:
        ...
        server.set_allowed_connections(["Track HW", "ui_3"])
        server.start_server(update_callback)
        root.socket_server = server
        
        connection_status.config(text="🟡 Test UI Server Ready", fg='orange')
        ...
        def wait_for_main_ui(attempt=0, max_attempts=10):
            if attempt >= max_attempts:
                connection_status.config(text="🔴 Main UI Not Found", fg='red')
                add_to_message_log("❌ Could not find Main UI after maximum attempts")
                return
            ...
            if check_port_open('localhost', 12345):
                add_to_message_log("✅ Main UI detected, attempting connection...")
                connect_to_main_ui()
            else:
        ...
        def connect_to_main_ui():
            try:
                ...
                if success:
                    connection_status.config(text="🟢 Connected to Main UI", fg='green')
                    add_to_message_log("✅ Successfully connected to Main UI")
                    ...
                    add_to_message_log("❌ Connection to Main UI failed")
                    
            except Exception as e:
                connection_status.config(text="🔴 Connection Error", fg='red')
        ...
        connection_status.config(text="🔴 Server Error", fg='red')
        return None
